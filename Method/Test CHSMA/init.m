...
for n = 1:h_number  
    ...
    h(n).archive.NP = h_len+1;
    h(n).archive.X = [];
    h(n).archive.F = [];
    
    h(n).Fm = 0.5; 
    h(n).Fm2  = 0.5; 
    [h(n).Factor , h(n).Factor2] = randFCR(h_len, h(n).Fm2, 0.1, h(n).Fm, 0.1);
...
%     h_len = (N-h_number)/h_number;
    h_len = (N)/h_number;
    start_range = h_number+1 + (h_len-1)*(n-1);
    m = zeros(1,h_len);
    m(1,1)= n;
    for counter = 2:h_len
        ...
        h(n).X(1,:) = X(SmellIndex(m(1,n)),:);
        h(n).F(1,1) = SmellOrder(m(1,n),1);
        h(n).pBest_X(1,:) = X(SmellIndex(n),:);
        h(n).pBest_F(1,1) = SmellOrder(n,1);
        h(n).pre_F(1,1) = SmellOrder(m(1,n),1);
        h(n).pre_X(1,:) = X(SmellIndex(m(1,n)),:);
                % initialize smell path archive
        r_temp = rand(1,dim);
        m_temp = r_temp>0.5; 
        N_temp = -1 * ones(1,dim);
        h(n).Levy_Archive(1,:) = (N_temp+m_temp)+ m_temp;

        h(n).Levy_Change_Order(1,:) = randperm(dim);
        h(n).Levy_Change_Counter(1,1) = 1; 
        h(n).path_flag(1,1) = 1 ;
    ...
    for i = 2:h_len
        ...
        h(n).pre_X(i,:) = X(SmellIndex(m(1,i)),:);
...
%         CMA.xmeanw =  mean(h(n).X(1,:))';
        CMA.sigma = 0.5; %0.25; % CMA.sigma = 0.25;
        CMA.minsigma = 1e-15;   %15;
