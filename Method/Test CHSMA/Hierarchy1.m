...
%                 h(n).X(1,:) = h(n).X(1,:) +  (tanh(1-it/Max_iter) *rand()*(-t_position - h(n).X(1,:))+ tanh(it/Max_iter)*rand()*(t_position - h(n).X(1,:))) +  tanh(it/Max_iter)*sum(levy(5,dim,beta)) ; %+ (it/Max_iter)*rand()*(bestPositions(1,:)- h(n).X(1,:));
                ...
                if rand()<factor   %> Max_iter/2
                    h(n).X(1,:) = h(n).CMA.arx(:,1)' +   a*rand()*(bestPositions - h(n).X(1,:)) + b*rand()*(-t_position - h(n).X(1,:)) ; % + c* rand()* (h(n).X(r1,:)-union);
                else                 %  rand()*b*(t_position - h(n).X(1,:))+
                    h(n).X(1,:) = h(n).CMA.arx(:,1)' +   rand()*a*(t_position - h(n).X(1,:))+ rand()*(h(n).X(r1,:)-union); %   +  b*sum(levy(5,dim,beta)) ; %+ (it/Max_iter)*rand()*(bestPositions(1,:)- h(n).X(1,:));
...
%     factor = (h(n).F(1) - h(n).F(i))/S;
    ...
    if rand() < factor  %|| h(1).rank(1,n) ==1 
        ...
        h(n).X(i,:) = h(n).CMA.arx(:,i)' + c*rand()*(h(h_n).pBest_X(r_i(1,1),:)-union);% + b* rand()*(h(h_n).pBest_X(r_i(1,2),:)-union) ;  % +  a* rand()*(h(n).X(1,:)-t_h(n).X(i,:)) 
