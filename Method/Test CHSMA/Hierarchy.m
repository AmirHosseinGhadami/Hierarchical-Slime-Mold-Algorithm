...
%                 h(n).X(1,:) = h(n).X(1,:) +  (tanh(1-it/Max_iter) *rand()*(-t_position - h(n).X(1,:))+ tanh(it/Max_iter)*rand()*(t_position - h(n).X(1,:))) +  tanh(it/Max_iter)*sum(levy(5,dim,beta)) ; %+ (it/Max_iter)*rand()*(bestPositions(1,:)- h(n).X(1,:));
                ...
                if rand()<factor   %> Max_iter/2
                    h(n).X(1,:) = h(n).X(1,:) +   rand()*(bestPositions - h(n).X(1,:));
                else 
...
%                 h(n).X(2,:) = h(n).X(2,:) + a*rand()*( h(n).X(1,:) - h(n).X(2,:) ) +b *rand()*(-t_position - h(n).X(2,:));
%                 h(n).X(3,:) = h(n).X(3,:)  + a* rand()*( h(n).X(2,:) - h(n).X(3,:) ) + b* rand()*(t_position - h(n).X(3,:));
...
%     factor = (h(n).F(1) - h(n).F(i))/S;
    if rand() < factor && it<4*Max_iter/5
        ...
        h(n).X(i,:) = h(n).CMA.arx(:,i)' + b*rand()*(h(h_n).pBest_X(r_i(1,1),:)-t_h(n).X(i,:)) ;  % +  a* rand()*(h(n).X(1,:)-t_h(n).X(i,:)) 
%         h(n).X(i,:) = h(n).X(i,:) + rand()*(h(h_n).X(r_i(1,1),:)-t_h(n).X(i,:)) ;
    ...
    for n = 1:h_number
         h(n) = adapt(h(n),dim,it,bestPositions);
    end
